...
from .dataInterface import interfaceCapture, interfaceKinged, interfaceGetLegalMove, interfaceGetLastBoard, CheckTakenPieces
...
class Board:
    ...
    def __init__(self, old_spots=None, the_player_turn=True):
        ...
        self.lastSpots = copy.deepcopy(self.spots)
    ...
    def get_captured_pieces(self):
        if self.player_turn == True :
            for i in range(8):
                for j in range(4):
                    #print(str(self.lastSpots[i][j]) + " " + str(self.spots[i][j]))
                    if self.lastSpots[i][j] == self.P1 and self.spots[i][j] == self.EMPTY_SPOT : 
                        interfaceCapture([i, j], "Player")
                        #print(str(i) + " " + str(j) + " is captured")
        if self.player_turn == False :
            for i in range(8):
                for j in range(4):
                    if self.lastSpots[i][j] == self.P2 and self.spots[i][j] == self.EMPTY_SPOT : 
                        interfaceCapture([i, j], "AI")

    def get_kinged_pieces(self):
        if self.player_turn == True :
            for j in range(4):
                    if self.lastSpots[0][j] != self.P2 and self.spots[0][j] == self.P2_K : 
                        interfaceKinged([0, j], "Player")
        if self.player_turn == False :
            for j in range(4):
                    if self.lastSpots[7][j] != self.P1 and self.spots[7][j] == self.P1_K : 
                        interfaceKinged([7, j], "AI")
    ...
    def print_board(self):
        ...
        """
        self.get_captured_pieces()
        self.get_kinged_pieces()
        self.lastSpots = copy.deepcopy(self.spots)
        # CheckTakenPieces(self.spots)
        interfaceGetLastBoard(self.lastSpots)
        index_columns = "   "
    ...
    def run(self):
        ...
        while not game_state.is_game_over() and num_moves < self.rules.max_moves:
            ...
            if not quiet:
                ...
                #input()
            interfaceGetLegalMove(game_state.get_legal_actions())
            if action is None:
